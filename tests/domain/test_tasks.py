...
class TaskGenerationTests(unittest.TestCase):
    def test_mask_can_number_uses_only_last_six_characters(self) -> None:
        ...
        self.assertEqual(mask_can_number(None), "Pending")
    ...
    def test_no_kyc_generates_high_priority_kyc_task(self) -> None:
        task = generate_member_tasks({**BASE_MEMBER, "kyc_status": "Not Started"}, BASE_FAMILY)[0]

        self.assertEqual(task.type, "kyc")
        self.assertEqual(task.priority, "high")
        self.assertEqual(task.description, "KYC not done")
        self.assertEqual(task.label, "Not Started")
    ...
    def test_registered_generates_medium_priority_re_kyc_task(self) -> None:
        task = generate_member_tasks({**BASE_MEMBER, "kyc_status": "Pending Re-KYC"}, BASE_FAMILY)[0]
    ...
    def test_payeezz_not_started_generates_high_priority_task(self) -> None:
        task = generate_member_tasks({**BASE_MEMBER, "payeezz_mandate_status": "Not Started"}, BASE_FAMILY)[0]
    ...
    def test_payeezz_pending_approval_generates_medium_priority_task(self) -> None:
        task = generate_member_tasks({**BASE_MEMBER, "payeezz_mandate_status": "Pending Approval"}, BASE_FAMILY)[0]

        self.assertEqual(task.type, "payeezz")
        self.assertEqual(task.priority, "medium")
        self.assertEqual(task.description, "PayEezz sent, awaiting acceptance")
        self.assertEqual(task.label, "Pending Approval")
    ...
    def test_verification_tasks_are_generated_with_expected_priorities(self) -> None:
        ...
        self.assertEqual(
            ...
            ["Pending Verification", "Pending Verification", "Pending Verification"],
        )
        self.assertEqual(
    ...
    def test_member_like_objects_are_supported(self) -> None:
        member = SimpleNamespace(**{**BASE_MEMBER, "kyc_status": "Not Started"})
